...
class Question():
    def __init__(self, question_txt, right_answer, wrong_1, wrong_2, wrong_3):
...
question_list = [
    Question("2 + 3 = ?", "5", "1", "2", "3"),
    Question("2 + 7 = ?", "9", "1", "2", "3"),
    Question("4 - 7 = ?", "-3", "4", "-5", "-4")
]

cur_question_list = question_list[:]
...
def next_question():
    global cur_question_list

    q = choice(cur_question_list)
    cur_question_list.remove(q)
    ask(q)

    if len(cur_question_list) == 0:
        cur_question_list = question_list[:]
